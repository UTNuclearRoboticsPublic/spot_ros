...
static std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> pointcloud_pub;
struct BoundingBox {
    double x_min{};
    double x_max{};
    double y_min{};
...
void filterPointcloud(sensor_msgs::msg::PointCloud2::UniquePtr pointcloud) {
    ...
    for (auto data_ptr = pointcloud->data.begin(); data_ptr != pointcloud->data.end(); std::advance(data_ptr, pointcloud->point_step)) {
        auto point_span = std::span<uint8_t>(data_ptr, pointcloud->point_step);
...
int main(int argc, char* argv[]) {
    ...
    auto nh = std::make_shared<rclcpp::Node>("spot_pointcloud_filter_node", opts);
    ...
    const std::string sensor_frame = nh->declare_parameter("sensor_frame", "velodyne");
    const std::vector<double> bounding_box_min = nh->declare_parameter("bounding_box_min_in_body", std::vector<double>({-0.20, -0.10, 0.10}));
    const std::vector<double> bounding_box_max = nh->declare_parameter("bounding_box_max_in_body", std::vector<double>({ 0.65,  0.10, 0.40}));
    ...
    try {
        ...
        if (sensor_tform_body.transform.rotation.w < 0.95) {
            RCLCPP_ERROR(nh->get_logger(), "Your lidar is not aligned with your robot. This code wasn't meant for that.");
            exit(1);
        }
    ...
    pointcloud_pub = nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_out", rclcpp::SensorDataQoS{});
    auto pointcloud_sub = nh->create_subscription<sensor_msgs::msg::PointCloud2>("cloud_in", rclcpp::SensorDataQoS{}, filterPointcloud);

    RCLCPP_INFO(nh->get_logger(), "Spot pointcloud filter online");
    rclcpp::spin(nh);
    return 0;
}
