...
class SimulatedRGBCamera:
    ...
    def renderingThread(names: list[str], colors: list[list[float]], vertices: list[np.ndarray], triangles: list[np.ndarray]):
        ...
        SimulatedRGBCamera.render_scene = open3d.visualization.rendering.OffscreenRenderer(SimulatedRGBCamera.max_width, SimulatedRGBCamera.max_height)
        ...
        for name, color, vertex_set, triangle_set in zip(names, colors, vertices, triangles):
            ...
            SimulatedRGBCamera.render_scene.scene.add_geometry(name, geometry, material)
        ...
        while rclpy.ok():
            ...
            # Render the image
            SimulatedRGBCamera.render_scene.setup_camera(intrinsic_matrix, extrinsic_matrix, image_width_px, image_height_px)
            open3d_image = SimulatedRGBCamera.render_scene.render_to_image()
