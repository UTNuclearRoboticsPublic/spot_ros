...
def populateTransformStamped(time: rclpy.time,
                             parent_frame: str,
                             child_frame: str,
                             transform: bosdyn.client.math_helpers.SE3Pose):
...
def getImageMsg(data, spot_wrapper):
    ...
    for frame_name in data.shot.transforms_snapshot.child_to_parent_edge_map:
        if data.shot.transforms_snapshot.child_to_parent_edge_map.get(frame_name).parent_frame_name:
            ...
            new_tf.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
    ...
    image_msg = Image()
    local_time = spot_wrapper.robotToLocalTime(data.shot.acquisition_time)
    image_msg.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
    ...
    local_time = spot_wrapper.robotToLocalTime(data.shot.acquisition_time)
    camera_info_msg.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
...
def GetJointStatesFromState(state, spot_wrapper):
    ...
    joint_state.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
...
def GetEStopStateFromState(state, spot_wrapper):
    ...
    for estop in state.estop_states:
        ...
        estop_msg.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
...
def GetOdomTwistFromState(state, spot_wrapper):
    ...
    local_time = spot_wrapper.robotToLocalTime(state.kinematic_state.acquisition_timestamp)
    twist_odom_msg.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
...
def GetTFFromState(state, spot_wrapper) -> TFMessage:
...
def GetBatteryStatesFromState(state, spot_wrapper):
    ...
    for battery in state.battery_states:
        ...
        battery_msg.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)

        battery_msg.identifier = battery.identifier
        battery_msg.charge_percentage = battery.charge_percentage.value
        battery_msg.estimated_runtime = rclpy.time.Time(battery.estimated_runtime.seconds, battery.estimated_runtime.nanos)
...
def GetPowerStatesFromState(state, spot_wrapper):
    ...
    power_state_msg.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
...
def getBehaviorFaults(behavior_faults, spot_wrapper):
    ...
    for fault in behavior_faults:
        ...
        new_fault.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
...
def getSystemFaults(system_faults, spot_wrapper):
    ...
    for fault in system_faults:
        ...
        local_time = spot_wrapper.robotToLocalTime(fault.onset_timestamp)
        new_fault.header.stamp = rclpy.time.Time(local_time.seconds, local_time.nanos)
        new_fault.duration = rclpy.time.Duration(fault.duration.seconds, fault.duration.nanos)
