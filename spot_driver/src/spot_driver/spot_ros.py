...
from geometry_msgs.msg import TwistWithCovarianceStamped, Twist, TwistStamped, Pose, PoseStamped
...
class SpotROS:
    ...
    def arm_goal_callback(self, goal):
        ...
        if not self.robot_allowed_to_move():
            rospy.logerr("arm action server received a request but motion is not allowed.")
            return

        success = True

        # Translate message, and execute trajectory while publishing feedback
        traj_point_positions, traj_point_velocities, time_since_ref = self.spot_wrapper.convert_ros_trajectory_msg(
            goal
        )
        ...
        self.spot_wrapper.arm_long_trajectory_executor(
            traj_point_positions, traj_point_velocities, time_since_ref
        )
    ...
    def finger_goal_callback(self, goal):
        ...
        if not self.robot_allowed_to_move():
            rospy.logerr("Finger action server received a request but motion is not allowed.")
            return
        ...
        self.spot_wrapper.finger_feedback_thread = threading.Thread(
            target=self.finger_follow_joint_trajectory_feedback
        )
        finger_feedback_thread.start()

        gripper_trajectory_executor_with_time_control(
            traj_point_positions, time_since_ref
        )
    ...
    def arm_follow_joint_trajectory_feedback(self):
        ...
        state = self.spot_wrapper.robot_state
        while not rospy.is_shutdown() and self.arm_feedback_publish_flag:
            # Get joint states

            # joint_states_source = GetJointStatesFromState(state, self.spot_wrapper)

            joint_states_source = self.spot_wrapper.get_joint_states()

            # print("===========")
            # print("Joint States: ", joint_states_source)
            # print("===========")

            self.arm_feedback.header.stamp = rospy.Time(
                # joint_states_source.header.stamp.secs, joint_states_source.header.stamp.nsecs
            ...
            # self.arm_feedback.joint_names = joint_states_source.name
            # self.arm_feedback.actual.positions = joint_states_source.position
            # self.arm_feedback.actual.velocities = joint_states_source.velocity
    ...
    def finger_follow_joint_trajectory_feedback(self):
        ...
        while not rospy.is_shutdown() and self.finger_feedback_publish_flag:
            # Get joint states
    ...
    def ee_vel_sub_callback(self, msg):
        ...
        if not self.robot_allowed_to_move():
            rospy.logerr("twist msg received but motion is not allowed.")
            return

        self.spot_wrapper.ee_velocity_msg_executor(msg)

    def ee_vel_sub_callback_prev(self, msg):
        """Callback for end effector velocity command subscriber"""
        if not self.robot_allowed_to_move():
            rospy.logerr("twist msg received but motion is not allowed.")
            return
            
        self.spot_wrapper.ee_velocity_msg_executor_prev(msg)
    ...
    # Moveit Traj-------------------------------------------------------------------
    # def handle_arm_joint_movetraj(self, srv_data: ArmJointMovementRequest):
    #    """ROS service handler to send joint movement to the arm to execute"""
    #    resp = self.spot_wrapper.arm_joint_move(joint_targets=srv_data.joint_target)
    #    return ArmJointMovementResponse(resp[0], resp[1])
    # Moveit Traj-------------------------------------------------------------------
    ...
    def main(self):
        ...
        #     "jog_arm/delta_jog_cmds", TwistStamped, self.ee_vel_sub_callback
        # )

        self.ee_vel_sub = rospy.Subscriber(
            "/filtered_twist", Twist, self.ee_vel_sub_callback_prev, queue_size=1
            # "/jog_arm/delta_jog_cmds_twist", Twist, self.ee_vel_sub_callback_prev            
        )
