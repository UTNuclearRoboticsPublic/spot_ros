...
class SpotWrapper:
    ...
    def verify_power_and_estop(self):
        if not self._robot.is_powered_on():
            self._robot.logger.info("Robot is not powered on. Attempting to power on.")
            self._robot.power_on(timeout_sec=20)
            assert self._robot.is_powered_on(), "Robot power on failed."
            self._robot.logger.info("Robot powered on.")
        else:
            self._robot.logger.info("Verified that robot is powered on.")
    ...
    def arm_long_trajectory_executor(
        ...
        self._robot.logger.info("Arm is about to move.")
        ...
        dt = time_since_ref[1] - time_since_ref[0]
        ...
        self._robot_command_client.robot_command(robot_cmd)
        traj_index = [0, 9]
        ...
        ref_time = seconds_to_timestamp(start_time)
        ref_time.seconds = ref_time.seconds

        while traj_index[0] < end_index:

            print("traj_index[0]" , traj_index[0])
            ...
            sleep_time = start_time + times[0] - (dt * 3 / 4) - time.time()
            if sleep_time > 0:
        ...
        time_since_ref[-1] - time_to_goal_in_seconds
        start_time - time.time()
    ...
    def gripper_trajectory_executor(self, traj_point_positions, time_since_ref):
        ...
        self._robot.logger.info("Gripper is about to move.")
    ...
    def ee_velocity_msg_executor(self, msg):    #TwistStamped
        ...
        angular_vel_max = 0.5
        linear_vel = np.clip(
            np.array([msg.twist.linear.x, msg.twist.linear.y, msg.twist.linear.z]) * scale,
            linear_vel_min,
            linear_vel_max,
        )
        angular_vel = np.clip(
            np.array([msg.twist.angular.x, msg.twist.angular.y, msg.twist.angular.z]) * scale,
            angular_vel_min,
            angular_vel_max,
        )
        ...
        end_effector_velocity = bosdyn.api.arm_command_pb2.ArmVelocityCommand.CartesianVelocity(
            frame_name=msg.header.frame_id, velocity_in_frame_name=linear               # Frame_name prev "body"
        )
    ...
    def ee_velocity_msg_executor_prev(self, msg):
        # Constraints and scale
        scale = 0.5
        drift = 0.1
        VELOCITY_CMD_DURATION = 0.3
        linear_vel_min = -0.2
        linear_vel_max = 0.2
        angular_vel_min = -0.5
        angular_vel_max = 0.5
        ...
        arm_velocity_command = bosdyn.api.arm_command_pb2.ArmVelocityCommand.Request(
            cartesian_velocity=end_effector_velocity,
            angular_velocity_of_hand_rt_odom_in_hand=angular,
            end_time = self._robot.time_sync.robot_timestamp_from_local_secs(time.time() + VELOCITY_CMD_DURATION)
        )
        ...
        try:
            self._robot_command_client.robot_command(robot_command, end_time_secs = time.time() + VELOCITY_CMD_DURATION)
        except (ResponseError, RpcError) as err:
    ...
    def get_joint_states(self):
        ...
        local_time = self.robotToLocalTime(
    ...
    @try_claim
    def velocity_cmd(self, v_x, v_y, v_rot, cmd_duration=0.3):
    ...
    def arm_stow(self):
        try:
            ...
            if not success:
                ...
                block_until_arm_arrives(self._robot_command_client, cmd_id, 3.0)
                time.sleep(2.0)
